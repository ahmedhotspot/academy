...
@section('title', 'إدارة المستخدمين - عرض')
                ...
                <div class="card border-0 shadow-sm mb-4">
                    <div class="card-body p-4">
                        <div class="d-flex flex-wrap justify-content-between align-items-center gap-3">
                            <div>
                                <p class="text-muted mb-1 small">بيانات المستخدم</p>
                                <h4 class="fw-bold mb-1">{{ $info['name'] ?? $user->name }}</h4>
                                ...
                                <div class="row g-3">
                                    <div class="col-6">
                                        <div class="p-3 rounded bg-success-subtle text-center">
                                            ...
                                            <h5 class="mb-0 text-success fw-bold">{{ $attendance['present'] ?? 0 }}</h5>
                                        </div>
                                    </div>
                                    <div class="col-6">
                                        <div class="p-3 rounded bg-danger-subtle text-center">
                                            <p class="small text-muted mb-1">غائب</p>
                                            <h5 class="mb-0 text-danger fw-bold">{{ $attendance['absent'] ?? 0 }}</h5>
                                        </div>
                                    </div>
                                    <div class="col-6">
                                        <div class="p-3 rounded bg-warning-subtle text-center">
                                            <p class="small text-muted mb-1">متأخر</p>
                                            <h5 class="mb-0 text-warning fw-bold">{{ $attendance['late'] ?? 0 }}</h5>
                                        </div>
                                    </div>
                                    <div class="col-6">
                                        <div class="p-3 rounded bg-info-subtle text-center">
                                            <p class="small text-muted mb-1">بعذر</p>
                                            <h5 class="mb-0 text-info fw-bold">{{ $attendance['excused'] ?? 0 }}</h5>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="col-xl-5">
                        <div class="card border-0 shadow-sm mb-3">
                            ...
                            <div class="card-body">
                                ...
                                <p class="mb-0">
                                    ...
                                    <span class="badge {{ $payroll['last_status_badge'] ?? 'bg-secondary' }}">{{ $payroll['last_status'] ?? '-' }}</span>
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="card border-0 shadow-sm">
                            <div class="card-header bg-white border-bottom d-flex align-items-center justify-content-between">
                                <h6 class="mb-0 fw-semibold"><i class="ti ti-bell me-1"></i> آخر الإشعارات</h6>
                                <span class="badge bg-warning text-dark rounded-pill">{{ $stats['unread_notifications_count'] ?? 0 }} غير مقروء</span>
                            </div>
                            <div class="card-body p-0">
                                @forelse($profile['notifications'] ?? [] as $notification)
                                    <div class="d-flex justify-content-between align-items-center p-3 border-bottom">
                                        <div>
                                            <p class="fw-semibold mb-1">{{ $notification['title'] }}</p>
                                            <small class="text-muted">{{ $notification['date'] }}</small>
                                        </div>
                                        <span class="badge bg-{{ $notification['type_color'] }}{{ $notification['is_read'] ? '' : ' text-dark' }}">
